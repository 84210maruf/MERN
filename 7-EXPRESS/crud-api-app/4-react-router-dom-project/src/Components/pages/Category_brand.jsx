...
function Category_brand() {
  const { brand } = useParams(); // Get category from URL
    ...
    <div className='bg-sky-50'>
      <div className="py-6">
        <fieldset className="border-t-2 mx-2 border-gray-600 rounded-xl">
          <legend className="px-6 text-2xl text-center">{brand.toUpperCase()} Collections</legend>
        </fieldset>
      </div>
      ...
      <article id="container" className="w-full py-2 md:py-4 lg:py-6 px-2 md:px-4 flex justify-center">
        ...
        >
          {products
            .filter(item => item.brand === brand) // Filter products by the dynamic category
            .map(item => (
